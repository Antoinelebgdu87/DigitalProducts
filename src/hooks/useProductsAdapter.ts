  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        if (!isConnected) {
          console.warn('🔄 Firebase connection issues detected, switching to fallback mode');
          setUseFailsafe(true);
        }
      } catch (error) {
        console.error('🔄 Firebase connection check failed, switching to fallback mode:', error);
        setUseFailsafe(true);
      }
    };
